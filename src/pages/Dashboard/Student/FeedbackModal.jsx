            ...
            <div className="modal-box p-0" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
                    <div className="form-control items-center">
                        <label className="label"><span className="label-text text-lg font-semibold">Your Overall Rating</span></label>
                        <Controller name="rating" control={control} rules={{ required: "A star rating is required." }} render={({ field }) => (<Rating style={{ maxWidth: 250 }} value={field.value || 0} onChange={field.onChange} itemStyles={ratingStyles} isRequired />)} />
                        {errors.rating && <span className="text-error text-xs mt-2">{errors.rating.message}</span>}
                    </div>
                    <div className="form-control">
                        <label className="label"><span className="label-text font-semibold">Your Review</span></label>
                        <textarea {...register("reviewText", { required: "Please share your experience." })} className="textarea textarea-bordered w-full h-32 text-base" placeholder="What did you like or dislike? What could be improved?"></textarea>
